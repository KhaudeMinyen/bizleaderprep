...
const VisualShowcase: React.FC<{ orgType: OrgType }> = ({ orgType }) => {
  ...
  const brandBorderHoverClass = orgType === 'FBLA' ? 'hover:border-rh-green/30' : orgType === 'DECA' ? 'hover:border-rh-cyan/30' : 'hover:border-rh-yellow/30';
  const brandTextHoverClass = orgType === 'FBLA' ? 'group-hover:text-rh-green' : orgType === 'DECA' ? 'group-hover:text-rh-cyan' : 'group-hover:text-rh-yellow';
